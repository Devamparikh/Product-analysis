...
<head>
  ...
  <title>GENERATE BILL | DASHBOARD</title>
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <div class="sidebar">
      ...
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        ...
        <div class="info">
          <a href="#" class="d-block">GOPAL TRADERS</a>
        </div>
      </div>
...
if(isset($_POST['customer_submit'])){
  ...
  echo $invoice_type = $_POST['invoice_type'];
  echo $paid_as = $_POST['paid_as'];
  
  if($_POST['bill_number'] == null){
  $query = "INSERT INTO `sale_entry` (`customer_name`, `customer_contact`, `sales_acc`, `bill_date`, `invoice_type`, `paid_as`) VALUES ('$customer_name', $customer_contact, '$sales_acc', '$bill_date', '$invoice_type', '$paid_as')";
  $result = mysqli_query($connection, $query);
  echo $bill_number = mysqli_insert_id($connection);
  echo $_SESSION["bill_number"] = $bill_number;
  
  $query_to_insert_billnumber = "UPDATE `sale_entry` SET `bill_number` = $bill_number WHERE `sale_entry`.`sale_id` = $bill_number";
...
if(isset($_POST['submit_product'])){
	if($_SESSION["bill_number"] != null){
        ...
        echo $product_discount = $_POST['product_discount'];
		echo $product_amount = $_POST['product_amount'];
		echo $product_quantity = $_POST['product_quantity'];
        
		$query = "INSERT INTO `sale_product` (`bill_number`, `product_description`, `product_varient`, `product_qty`, `per_product_rate`, `product_amount`, `product_discount`) VALUES ($bill_number, '$product_name', '$varient', $product_quantity, $product_rate, $product_amount, $product_discount)";
...
if(isset($_SESSION['bill_number'])){
  ...
  $query = "SELECT * FROM `sale_entry` WHERE `bill_number` = $bill_number";
  $result = mysqli_query($connection, $query);
...
?>
          <div class="card-body">
              <div class="row">
                    <div class="col-lg-4" id="CUSTOMER">
                        <span>NAME:</span>
                        <input type="text" name="customer_name" class="input" value="<?php if(isset($_SESSION['bill_number'])){
                        echo  $customer_name; 
                        } ?>" >
                    </div>
                    <div class="col-lg-4" id="SUPPLIER">
                        <span>SALES A/C:</span>
                        <input type="text" name="sales_acc" id="sales_acc" class="input" value="<?php if(isset($_SESSION['bill_number'])){echo $sales_acc;} ?>">
                  ...
                  <div class="col-lg-4">
                    <div id="SUPPLIER">
                        <span>MOBILE NO:</span>
                        <input type="number" name="customer_contact" class="input" value="<?php if(isset($_SESSION['bill_number'])){echo $customer_contact;} ?>">
                    </div>
                  </div>
              </div>
              ...
              <div class="row" style="text-align: center">
                <div class="col-lg-3" id="SUPPLIER">
                        <span>BILL DATE:</span>
                        <input type="date" name="bill_date" class="input" value="<?php if(isset($_SESSION['bill_number'])){echo $bill_date;}else{echo date('Y-m-d');} ?>">
                </div>
                <div class="col-lg-3" >                      
                        <span>BILL NO:</span>
...
-->
              ...
              <div style="text-align: center">
                  <button type="submit" class="btn col-lg-2 btn-primary" name="customer_submit">Submit</button>
              </div>
                ...
                <div class="row">
                    <div class="col-lg-3">
                        ...
                        <span>QUANTITY:</span><br>
                            <input type="text" name="product_quantity" class="input" id="product_quantity"><br>
                        <span>DISCOUNT (in %):</span><br>
                            <input type="text" name="product_discount" class="input" id="product_discount"><br>
                        <span>RATES:</span><br>
                            <input type="number" name="product_rate" class="input" id="product_rate"><br>
                        <span>AMOUNT:</span><br>
                            <input type="number" step="any" name="product_amount" class="input" id="product_amount"><br><br>
                            <script>
                              $(document).ready(function(){
                         ...
                         <script>
                              ...
                              $("#product_rate").keyup(function() {
                                ...
                                var amount = (qty * rate);
                                var new_disc = (amount*disc)/100;
                                amount = amount - new_disc;
                                $("#product_amount").val(amount);
                              });
                            </script>
                        
                        <div style="text-align: right">
                            <button type="submit" class="btn btn-primary" name="submit_product">ADD -></button>
                        </div>
                    </div> 
                    </form>
                    <div class="col-lg-9">
                            ...
                            <div class="col-12 table-responsive">
                              <table class="table table-striped">
                                ...
                                $amount = 0;
	                              if(isset($_SESSION['bill_number'])){
                                  $bill_number = $_SESSION['bill_number'];
                                  $query = "SELECT * FROM `sale_product` WHERE `bill_number` = $bill_number";
                                  $result = mysqli_query($connection, $query);
                                  ...
                                  while($row = mysqli_fetch_assoc($result)){
                                    ...
                                    echo "<td><button class='btn btn-danger' name='' ><a href='GENERATE_BILL.php?delete={$sale_product_id}' style='text-decoration : none; color : #fff;'>x</a></button></td>";
                                    echo "</tr>";
                                  }
                                }
...
<?php 
    ...
    if(isset($_GET['delete'])){
        ...
        echo $the_sale_product_id = $_GET['delete'];
        
        $delete = " DELETE FROM sale_product 
        WHERE sale_product_id = {$the_sale_product_id } ";
...
if(isset($_POST['cancel'])){
  if(!empty($bill_number)){
    ...
    $delete = " DELETE FROM sale_product WHERE bill_number = {$bill_number} ";
    $delete_query = mysqli_query($connection,$delete);
    $delete = " DELETE FROM sale_entry WHERE bill_number = {$bill_number} ";
    $delete_query = mysqli_query($connection,$delete);
...
if(isset($_POST['save'])){
  if(!empty($bill_number)){
    if(!empty($amount_array[1])){
      $save_total ="UPDATE `sale_entry` SET `total` = $amount, `cgst` = $cgst, `sgst` = $sgst, `round_off` = $amount_array[1], `payable_amount` = $payable_amount WHERE `purchase_entry`.`bill_number` = $bill_number";
    }else{
      $save_total ="UPDATE `sale_entry` SET `total` = $amount, `cgst` = $cgst, `sgst` = $sgst, `round_off` = 0, `payable_amount` = $payable_amount WHERE `purchase_entry`.`bill_number` = $bill_number";
    }
...
if(isset($_POST['print'])){
  if(!empty($bill_number)){
    ...
    unset($_SESSION['bill_number']);
    echo "<script language='javascript'>window.open('INVOICE-SE.php?print=$print_bill','_blank');</script>";
    echo "<script language='javascript'>window.open('GENERATE_BILL.php','_self');</script>";
